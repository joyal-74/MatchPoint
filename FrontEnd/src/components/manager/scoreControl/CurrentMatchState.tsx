...
import type { BattingStats, BowlingStats, Match, Team } from '../../../domain/match/types';
...
const CurrentMatchState: React.FC<CurrentMatchStateProps> = ({ match, teamA, teamB, liveScore }) => {
    ...
    const strikerStats = battingStatsArray.find((s: BattingStats) => s.playerId === currentInnings.currentStriker) || { runs: 0, balls: 0, fours: 0, sixes: 0 };
    const nonStrikerStats = battingStatsArray.find((s: BattingStats) => s.playerId === currentInnings.currentNonStriker) || { runs: 0, balls: 0, fours: 0, sixes: 0 };
    const bowlerStats = bowlingStatsArray.find((s: BowlingStats) => s.playerId === currentInnings.currentBowler) || { overs: 0, maidens: 0, wickets: 0, runsConceded: 0 };
