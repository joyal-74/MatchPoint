...
import { fetchManagerDetails } from '../../features/admin/users';
import LoadingOverlay from '../../components/shared/LoadingOverlay';
import type { ManagerDetails } from '../../features/admin/users/userTypes';

const sampleManager: ManagerDetails = {
...
const ManagerDetails = () => {
    const [manager, setManager] = useState<ManagerDetails>(sampleManager);
    ...
    useEffect(() => {
        const getManagerDetails = async () => {
            if (id) {
                try {
                    setLoading(true);
                    const result = await dispatch(fetchManagerDetails(id)).unwrap();
                    setManager(result);
                } catch (error) {
                    console.error("Failed to fetch manager details:", error);
                    setManager(sampleManager);
                } finally {
                    setLoading(false);
                }
            } else {
                    ...
                    <div className="max-w-6xl mx-auto bg-neutral-800 rounded-2xl shadow-2xl p-6 border border-neutral-700">
                        ...
                        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-6">
                            <div className="relative">
                                <img
                                    src={manager.profileImage || '/office-worker.png'}
                                    alt="Manager Profile"
                            ...
                            <div className="text-center sm:text-left flex-1">
                                ...
                                <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                                    ...
                                    <span
                                        className={`px-2 py-1 rounded-full text-xs font-medium border ${manager.isBlocked
                                ...
                                <p className="text-neutral-400 text-sm flex items-center justify-center sm:justify-start gap-1">
                                    ...
                                    Joined: {manager.joinedAt}
                                </p>
                            </div>

                            <button
                                className={`px-4 py-2 rounded-lg text-white font-medium transition-colors duration-200 text-sm ${manager.isBlocked
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 rounded-lg p-4 border border-emerald-500/20">
                                    ...
                                    <p className="text-3xl font-bold text-emerald-400">
                                        {manager.stats.tournamentsCreated}
                                    </p>
                                </div>
                                ...
                                <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 rounded-lg p-4 border border-blue-500/20">
                                    ...
                                    <p className="text-3xl font-bold text-blue-400">
                                        {manager.stats.tournamentsParticipated}
                                    </p>
                                </div>
