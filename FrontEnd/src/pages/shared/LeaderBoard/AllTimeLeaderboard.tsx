...
import type { PlayerRole, LeaderboardRow, TimePeriod } from "./leaderboardData"; 
...
const AllTimeLeaderboard: React.FC = () => {
    ...
    const leaderboardRows: LeaderboardRow[] = useMemo(() => {
        ...
        return leaderboard.map((player: any, idx: number) => ({
            Rank: (page - 1) * 10 + idx + 1,
            Name: player.name || "Unknown",
            Match: player.matches || player.match || 0,
            Role: (player.role as LeaderboardRow["Role"]) || "Batter",
            Runs: String(player.runs || "0"),
            "100": player.hundreds ?? "-",
            "50": player.fifties ?? "-",
            "Bat. average": player.average || 0,
            "Strike rate": player.strikeRate || 0,
            Best: player.best || "-",
        }));
    }, [leaderboard, page]);
                    ...
                    <div className="flex bg-muted/50 p-1 rounded-lg border border-border w-full md:w-auto overflow-x-auto">
                        {roleOptions.map((role) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 whitespace-nowrap flex-1 md:flex-none ${
                                    selectedRole === role
                ...
                <section className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
                        ...
                        <table className="min-w-full divide-y divide-border">
                            <thead>
                                <tr className="bg-muted/50">
                                    {headers.map((header) => (
                                        <th key={header} className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider whitespace-nowrap">
                                            {header}
                                        </th>
                                    ))}
                                </tr>
                            ...
                            <tbody className="divide-y divide-border bg-card">
                                {!loading && leaderboardRows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-muted/30 transition-colors duration-150 group">
                                        {headers.map((header) => {
                                            // FIX 2: Explicitly access row by header name to match your LeaderboardRow interface
                    ...
                    <div className="border-t border-border px-6 py-4 flex items-center justify-between bg-muted/20">
                        ...
                        <button
                            ...
                            disabled={!leaderboard || leaderboard.length < 10}
                            className="flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
                        >
